...
word = ft.word(id,list_word)
space = ['__'] *len(word)
print('  '.join(space))
n=0
while n != 7:
    letter=input("entrer deviner una lettre du mot ")
    if letter in space :
        print("You have already found this letter, enter another one. ")
    else :
        ...
        if letter in word :
            ...
            n = n+1
        if  ft.space_fill(space,word,letter)== word :
            break
        ft.affichage(n)
        print()
        print(tempon)
        print()
        ft.space_fill(space,word,letter)

if n < 7 :
    ft.affichage(8)
else:
    print("the search word was :", word)

...
def connexion():
    ...
    label['bg']='gray'
    def connect():
        with open("data.json","r",encoding="utf-8") as file:    # pour ouvrir la base de donne 
            data = json.load(file)
            i=0
        for compte in data : # ici c;est pour determiner si l'utilisateur avait deja un compte
            if nom_utilisateur.get() != (compte["nom"]) or prenom_utilisateur.get() != (compte["prenom"]) :
                i =i+1    #ici on veut savoir le nombre de personne dont les informations est differentes de ceux entrer par l'utilisateur 
            else:
                ...
                if mots==((compte["mot de pass"])):
                    ...
                    label['text']= mot
                break  # pour arreter la boucle pour au moment ou on trouve le le comptte correspondand parceque le elses'execute seleument si ls informations entrer par l'utilisateur conresponde a celui d'un compte de la base de donne donc sa ne sert plus a rien de verifier les autres comptes 

        if i== len(data):  # verifie que le mot de passe ne correspond a aucun a=compte utilisateur et comparen i avec le nombre de de compte d'utilisateur de notre base de donne 
                            #
